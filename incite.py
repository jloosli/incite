...
sys.path.append(os.path.join(theDir, "Adafruit-Raspberry-Pi-Python-Code"))
...
print 'Displaying {} channels'.format(displayChannels)
display = 0
...
while 1:
  ...
  for i in range(0,4):
    ...
    ch[i]=result
  print "{0:.3f} {1:.3f} {2:.3f} {3:.3f}".format(*ch)
  ...
  if hasGrid:
    ...
    for i in range(displayChannels):
      steps[i] = int(math.floor(ch[i] / maxChannelV * 64 / displayChannels))
